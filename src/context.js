...
export const AppProvider = ({children}) =>{
    ...
    const fetchData = async (url)=>{
        ...
        if (response.ok){
          ...
          let tempA = Math.round(
            info.forecast.forecastday[0].hour[
              countryHour + 1 >= 24 ? countryHour + 1 - 24 : countryHour + 1
            ].temp_c
          );
          let tempB = Math.round(
            info.forecast.forecastday[0].hour[
              countryHour + 2 >= 24 ? countryHour + 2 - 24 : countryHour + 2
            ].temp_c
          );
          let tempC = Math.round(
            info.forecast.forecastday[0].hour[
              countryHour + 3 >= 24 ? countryHour + 3 - 24 : countryHour + 3
            ].temp_c
          );
          let tempD = Math.round(
            info.forecast.forecastday[0].hour[
              countryHour + 4 >= 24 ? countryHour + 4 - 24 : countryHour + 4
            ].temp_c
          );
          let id = uuidv4();
          let condA =
            info.forecast.forecastday[0].hour[
              countryHour + 0 >= 24 ? countryHour + 0 - 24 : countryHour + 0
            ].condition.text;
          let condB =
            info.forecast.forecastday[0].hour[
              countryHour + 1 >= 24 ? countryHour + 1 - 24 : countryHour + 1
          ...
          setState([
            ...state,
            {
              ...
              isDay,
              nextWeather: {
                weatherA: [isDayA, condA, tempA],
                weatherB: [isDayB, condB, tempB],
                weatherC: [isDayC, condC, tempC],
                weatherD: [isDayD, condD, tempD],
              },
            },
          ]);
          console.log(info);
